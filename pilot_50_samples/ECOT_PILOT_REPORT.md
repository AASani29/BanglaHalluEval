...
**Status:** Pilot complete; full-benchmark scaffolding ready in [`../full_ecot_run/`](../full_ecot_run/). See [`../full_ecot_run/LAB_SETUP_GUIDE.md`](../full_ecot_run/LAB_SETUP_GUIDE.md) for the lab-PC guide and [`../full_ecot_run/RUNPOD_GUIDE.md`](../full_ecot_run/RUNPOD_GUIDE.md) for the RunPod path. Project-wide overview: [`../HALLUCINATION_MITIGATION.md`](../HALLUCINATION_MITIGATION.md).
...
## 7.3 Code already prepared for the full run

The pilot evaluator is replaced by a production scaffold in `full_ecot_run/`. Every per-model wrapper imports from a shared `_ecot_core.py` so the prompt logic, JSON parsing, aggregation, and **task-aware verdict policy** stay in one place and apply to all 7 judges.

| File | Purpose |
|---|---|
| `full_ecot_run/scripts/_ecot_core.py` | shared driver: prompt builder, JSON parser, `aggregate_verdict`, task-aware `is_hallucinated`, resumable streaming CSV writer (checkpoint every 20 rows) |
| `full_ecot_run/scripts/_backend_ollama.py` | Ollama HTTP client with block-and-retry on connection drops (mirrors the existing `scripts/evaluate_cot_ollama.py` behavior) |
| `full_ecot_run/scripts/_backend_openai.py` | OpenAI Responses-API client with `response_format=json_object` |
| `full_ecot_run/scripts/_backend_tigerllm.py` | HuggingFace Transformers loader for `md-nishat-008/TigerLLM-9B-it` (bfloat16, single GPU) |
| `full_ecot_run/scripts/02_run_*.py` (×7) | thin per-judge wrappers — set model name + token budgets, call `run_all()` |
| `full_ecot_run/scripts/03_compute_metrics.py` | per-judge BHS table + cross-judge `_all_judges_metrics.csv` |
| `full_ecot_run/scripts/04_build_paper_tables.py` | markdown + LaTeX tables for the rebuttal body |
| `full_ecot_run/scripts/run_all_open_weight.sh` | sequential driver for the 6 open-weight judges on the lab PC |

All scripts default to **`--resume`** (skip rows whose `id_key` is already in the output CSV). Every script accepts `--task qa|summarization|reasoning|all` and `--track A|B|both`.

## 7.4 Recommended execution order (3-week rebuttal budget)

| Week | Milestone | Where |
|---|---|---|
| **W1, day 1** | Run GPT-4.1 mini full benchmark (validates the scaffold at scale) | dev machine → `python full_ecot_run/scripts/02_run_gpt4_1_mini.py --task all --track both` |
| **W1, days 2–3** | 200-sample re-validation of verdict-source policy on **LLaMA-3.1-8B** and **TigerLLM-9B** | lab PC; reuse pilot sampler — `01_sample_pilot.py --n 200`. If policy holds, keep `VERDICT_SOURCE` as-is. If a judge inverts the QA or Reasoning rule, fork per-judge constants in `_ecot_core.py`. |
| **W1, days 4–5** | Full LLaMA-3.1-8B + TigerLLM-9B runs | lab PC; resume-safe — start before leaving for the day. |
| **W2** | Full Mistral-Nemo-12B, DeepSeek-R1-14B, Gemma-2-27B, Qwen2.5-32B runs | lab PC; run with `run_all_open_weight.sh` or one-by-one to free the GPU between. |
| **W3** | `03_compute_metrics.py` + `04_build_paper_tables.py` + rebuttal write-up | dev machine. |
