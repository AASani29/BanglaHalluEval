#!/usr/bin/env python3
"""Evaluate QA, Summarization, and Reasoning hallucinations using BanglaLLM/bangla-llama-13b-instruct-v0.1.
...
Usage:
    python scripts/evaluate_banglallama.py --task all
    python scripts/evaluate_banglallama.py --task qa
    python scripts/evaluate_banglallama.py --task summ3000
    python scripts/evaluate_banglallama.py --task summ1000
    python scripts/evaluate_banglallama.py --task reasoning
    python scripts/evaluate_banglallama.py --task reasoning_gt
"""
...
def generate_batch(prompts: list, tokenizer, model, max_new_tokens: int = 16) -> list:
    prompts = [_format_prompt(tokenizer, prompt) for prompt in prompts]
    inputs = tokenizer(
        prompts,
        return_tensors="pt",
        padding=True,
...
def run_qa(tokenizer, model, resume: bool) -> None:
    ...
    output_file = "QA/Results/qa_4000_labeled_banglallama.csv"
...
TASKS = {
    ...
    "reasoning_gt": lambda tok, mod, resume: run_reasoning(
        ...
        "Reasoning/Results/reasoning_evaluation_scored_groundtruth_banglallama.csv",
...
def main():
    ...
    p.add_argument("--quantize", dest="quantize", action="store_true", default=True, help="Load the model in 4-bit")
    p.add_argument("--no-quantize", dest="quantize", action="store_false")
    args = p.parse_args()

    tokenizer, model = load_model(quantize=args.quantize)
